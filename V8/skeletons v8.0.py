...
module <MODULE_NAME> #(parameter TICKS_PER_BAUD = <WIDTH>) (
    ...
    "async_fifo": {
        ...
        "notes": (
            "Asynchronous FIFO physical contract. Freezes the CDC essentials and "
            "requires a synchronous dual-port RAM read so synthesis preserves a "
            "memory cell instead of exploding into flops/muxes."
        ),
        "invariants": [
            "__fn:async_fifo_synchronous_memory",
            r"gray",
            r"sync",
            r"always\s*@\s*\(\s*posedge\s+wr_clk",
            r"always\s*@\s*\(\s*posedge\s+rd_clk",
            r"assign\s+full\s*=",
            r"assign\s+empty\s*=",
        ],
        "skeleton": """\
...
//   - Do NOT reset the mem array.
//   - Do NOT read mem through assign/wire/always @(*) logic.
//   - Keep memory read/write in reset-free clocked blocks for Yosys inference.
//   - N+1 bit pointers ([ADDR_W:0]) are mandatory to distinguish full from empty.
//   - rd_data must be a registered output loaded inside always @(posedge rd_clk).
//   - full/empty are combinational assigns from registered Gray pointers only.
module <MODULE_NAME> #(
    ...
    reg [WIDTH-1:0] mem [0:DEPTH-1]; // DO NOT RESET THIS ARRAY.

    // FROZEN: N+1 bit pointers are mandatory for async FIFO wraparound/full detection.
    ...
    reg [ADDR_W:0] rd_ptr_gray_wrclk1;
    reg [ADDR_W:0] rd_ptr_gray_wrclk2;
    reg [ADDR_W:0] wr_ptr_gray_rdclk1;
    reg [ADDR_W:0] wr_ptr_gray_rdclk2;
    ...
    wire [ADDR_W:0] rd_ptr_bin_plus1 = rd_ptr_bin + 1'b1;
    wire [ADDR_W:0] wr_ptr_gray_plus1 = bin2gray(wr_ptr_bin_plus1);
    wire [ADDR_W:0] rd_ptr_gray_plus1 = bin2gray(rd_ptr_bin_plus1);

    // Combinational full uses the current write pointer. Using wr_ptr+1 here
    // while also gating writes with !full incorrectly reduces depth to 15.
    assign full = (wr_ptr_gray ==
                   {~rd_ptr_gray_wrclk2[ADDR_W:ADDR_W-1],
                    rd_ptr_gray_wrclk2[ADDR_W-2:0]});
...
def _run_custom_invariant(fn_name: str, v_code: str) -> tuple:
    ...
    if fn_name == "async_fifo_synchronous_memory":
        clean_code = _strip_verilog_comments(v_code)
        mem = re.search(
            r'\breg\s*\[[^\]]+\]\s+([A-Za-z_]\w*)\s*\[[^\]]+\]',
            clean_code,
            re.IGNORECASE,
        )
        if not mem:
            return False, "Async FIFO must declare a 2D reg memory array"
        mem_name = mem.group(1)
        mem_ref = rf'{re.escape(mem_name)}\s*\['
        text = re.sub(r'\s+', '', clean_code).lower()
        mem_l = mem_name.lower()

        if re.search(rf'\bassign\w*=[^;]*{re.escape(mem_l)}\[', text):
            return False, "Async FIFO must not use assign-based asynchronous memory reads"
        if re.search(rf'\bwire(?:\[[^\]]+\])?\w*=[^;]*{re.escape(mem_l)}\[', text):
            return False, "Async FIFO must not use wire-initializer asynchronous memory reads"

        has_wr_clk_block = False
        has_rd_clk_block = False
        has_sync_write = False
        has_sync_read = False
        ptr_n_bit_decl = re.search(
            r'\breg\s*\[\s*(?:ADDR_W|ADDR_WIDTH|PTR_W|PTR_WIDTH)\s*-\s*1\s*:\s*0\s*\]'
            r'\s+[^;]*(?:wr|rd)[^;]*(?:ptr|bin|gray)',
            clean_code,
            re.IGNORECASE,
        )
        if ptr_n_bit_decl:
            return False, "Async FIFO pointers must be N+1 bits wide, not [ADDR_W-1:0]"
        ...
        def _is_n_plus_one_width(width_expr: str) -> bool:
            w = re.sub(r'\s+', '', width_expr).lower()
            if w in {"addr_w:0", "addr_width:0", "ptr_w:0", "ptr_width:0", "4:0"}:
                return True
            if re.fullmatch(r'[a-z_]\w*:0', w) and "addr" in w:
                return True
            return False

        wr_has_n_plus_one = False
        rd_has_n_plus_one = False
        for width_expr, names_expr in decls:
            if not _is_n_plus_one_width(width_expr):
                continue
            names = [n.strip().lower() for n in names_expr.split(",")]
            if any(n.startswith("wr") and any(tok in n for tok in ("ptr", "bin", "gray")) for n in names):
                wr_has_n_plus_one = True
            if any(n.startswith("rd") and any(tok in n for tok in ("ptr", "bin", "gray")) for n in names):
                rd_has_n_plus_one = True

        if not (wr_has_n_plus_one and rd_has_n_plus_one):
            return False, "Async FIFO must declare N+1 bit write/read pointers, e.g. reg [ADDR_W:0] wr_ptr_bin and rd_ptr_bin"

        for header, block in _iter_simple_always_blocks(clean_code):
            header_l = header.lower()
            block_l = block.lower()
            block_compact = re.sub(r'\s+', '', block_l)

            is_comb = "*" in header_l
            if is_comb and f"{mem_l}[" in block_compact:
                return False, "Async FIFO must not read memory in combinational always logic"

            has_mem_access = bool(re.search(mem_ref, block, re.IGNORECASE))
            if has_mem_access and ("negedge" in header_l or "rst" in header_l or "reset" in header_l):
                return False, "Async FIFO memory access blocks must be clocked and reset-free"

            if "posedge" in header_l and "wr_clk" in header_l:
                has_wr_clk_block = True
                if has_mem_access and "<=" in block:
                    has_sync_write = True

            if "posedge" in header_l and "rd_clk" in header_l:
                has_rd_clk_block = True
                if re.search(rf'\b[A-Za-z_]\w*\s*<=\s*{mem_ref}', block, re.IGNORECASE | re.DOTALL):
                    has_sync_read = True
            ...
            if reset_m and re.search(mem_ref, reset_m.group('body'), re.IGNORECASE):
                return False, "Async FIFO memory array must not be reset or cleared"

        if not has_wr_clk_block:
            return False, "Async FIFO missing always @(posedge wr_clk) write domain block"
        if not has_rd_clk_block:
            return False, "Async FIFO missing always @(posedge rd_clk) read domain block"
        if not has_sync_write:
            return False, "Async FIFO memory write must occur inside the wr_clk block"
        if not has_sync_read:
            return False, "Async FIFO rd_data/read register must be loaded from memory inside the rd_clk block"
        return True, "ok"
