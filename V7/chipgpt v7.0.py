...
{error_context}{skeleton_section}{protocol_section}{hierarchy_section}{algorithm_section}
...
def verify_with_yosys(
    ...
    memoryish = plan.get("contains_memory", False) or (
        prompt_memoryish and not top_is_hierarchy
    )
    read_cmds = yosys_read_commands(plan["files"] + [v_file])
    if memoryish:
        yosys_script = (
            f"{read_cmds} hierarchy -top {design_name}; proc; opt; memory_dff; memory_collect; "
            "check -assert; stat"
        )
    else:
        yosys_script = (
...
def autonomous_build_loop(
    ...
    for attempt in range(max_retries):
        ...
        for v_code, tb_code in candidates:
            ...
            passed, log = verify_with_yosys(v_file, base_prompt, design_name, design_type)
            if not passed:
                err = extract_primary_error(log)
                print(f"      ❌ Yosys: {err}")
                current_score = (2, 0)
