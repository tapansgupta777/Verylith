...
def classify_failure(log: str) -> str:
    ...
    if "width" in l or "size" in l or "bit extraction" in l or "expects" in l:
...
- FSM_OUTPUT_WRONG       → a state machine output has the wrong value.
                           Common causes for UART TX:
                           (1) START state: tx_out must be 0 (low). If tx_out=1, FSM
                               is not entering START or reset is overriding output.
                           (2) DATA state: tx_out = tx_data_reg[bit_index] LSB-first.
                               If wrong bit, check bit_index starts at 0 and increments.
                           (3) STOP state: tx_out must be 1 (high).
                           (4) Baud counter: count from 0 to CLKS_PER_BIT-1 (inclusive)
                               before transitioning. Transition when counter==CLKS_PER_BIT-1.
                           (5) State not updating: ensure state<=next_state in sequential block.
- WIDTH_MISMATCH         → signal has wrong bit width for its context.
                           UART TX specific:
                           (1) bit_index must be reg [2:0] — exactly 3 bits — to index
                               an 8-bit register tx_data_reg[bit_index]. A 4-bit index
                               causes Verilator WIDTH error. NEVER use 4-bit for 8-bit indexing.
                           (2) baud_counter must be wide enough for CLKS_PER_BIT-1.
                               Use reg [7:0] or larger. Declare separately from bit_index.
                           Fix: reg [2:0] bit_index; reg [7:0] baud_counter;
...
def autonomous_build_loop(
    ...
    for attempt in range(max_retries):
        ...
        if skip_reviewer:
            print(f"   🔎 Reviewer: Skipped (skeleton design — invariant checker handles structure).")
