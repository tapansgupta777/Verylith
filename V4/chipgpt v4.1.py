...
def generate_rtl(
    ...
    cand_idx:        int   = 0,
    attempt_round:   int   = 0,   # generation round — ensures fresh calls per round
) -> dict:
    # Only cache fresh (non-mutation) calls. Mutations are intentionally unique.
    # attempt_round is included in the key so generation N never reuses generation N-1
    # RTL even when cand_idx is the same — breaks the same-output deadlock.
    is_mutation = bool(previous_error and best_v_code)
    cache_key   = get_cache_key(prompt, architecture, cand_idx, attempt_round) if not is_mutation else None
...
CRITICAL PORT NAMING RULE:
The port names above are the ONLY valid signal names.
Do NOT invent or rename ports (e.g. 'valid_out' instead of 'm_valid', 'data_in' instead of 's_data').
Your golden model and all checks MUST use the exact names from the PORT INTERFACE above.
...
def autonomous_build_loop(
    ...
    for attempt in range(max_retries):
        ...
        # When no valid RTL has been established yet, code_hashes accumulates
        # rejected candidates that block re-generation. Clear it each round so
        # the generator isn't penalized for previously discarded output.
        if best_v_code is None:
            code_hashes = set()
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=candidates_per_round) as executor:
            ...
            for i in range(candidates_per_round):
                ...
                futures[executor.submit(
                    ...
                    dynamic_idx, attempt   # attempt_round for cache busting
                )] = i
            ...
            try:
                for future in concurrent.futures.as_completed(futures, timeout=420):
                    ...
                    # Skeleton invariant check — two tiers:
                    # HARD: structural problems that guarantee simulation failure
                    #       (missing reset, missing sensitivity list, missing assign s_ready)
                    # SOFT: signal naming differences that may still be functionally correct
                    #       (buf_valid vs buffer_valid) — warn but let testbench decide
                    if design_type != "generic":
                        inv_ok, violations = check_skeleton_invariants(v_code, design_type)
                        if not inv_ok:
                            hard = [v for v in violations if "s_ready RHS missing" not in v]
                            soft = [v for v in violations if "s_ready RHS missing" in v]
                            if hard:
                                print(f"   ⚠️  Skeleton hard invariant violated ({hard[0][:60]}) — discarding.")
                                continue
                            if soft:
                                print(f"   ℹ️  Skeleton soft warning ({soft[0][:60]}) — passing to testbench.")
