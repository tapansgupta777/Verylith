...
def _run_custom_invariant(fn_name: str, v_code: str) -> tuple:
    ...
    if fn_name == "s_ready_uses_buf_valid_and_m_ready":
        ...
        if "m_ready" not in rhs:
            return False, (
                f"s_ready equation is wrong: got 'assign s_ready = {rhs[:40]}'. "
                f"MUST be: assign s_ready = !buf_valid || m_ready; "
                f"(drop m_ready and s_ready goes low when buffer empties — breaks throughput)"
            )
        ...
        if not re.search(r'[!~]\s*\w', rhs):
            return False, (
                f"s_ready equation is wrong: got 'assign s_ready = {rhs[:40]}'. "
                f"MUST be: assign s_ready = !buf_valid || m_ready; "
                f"(need negated buffer term so s_ready drops when buffer is full AND m_ready=0)"
            )
        return True, "ok"
