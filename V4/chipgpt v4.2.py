...
- HANDSHAKE_LOGIC        → a ready/valid handshake signal has the wrong value.
                           MANDATORY: s_ready MUST be driven as a combinational assign:
                             assign s_ready = !buf_valid || m_ready;
                           This is the ONLY correct equation for a 1-entry skid buffer.
                           Do NOT use: assign s_ready = !buffer_full; (drops m_ready)
                           Do NOT use: assign s_ready = m_ready;       (drops buffer term)
                           Do NOT register s_ready in always @(posedge clk).
                           If m_valid/m_data are wrong: check drain path priority —
                           drain (buf_valid→output) must fire before pass-through (s_valid→output).
