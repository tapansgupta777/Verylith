...
def validate_architecture(
    ...
    if any((not n) or n in PORT_NAME_STOPWORDS for n in port_names):
        return False
    if expected_port_names:
...
{error_context}{skeleton_section}{protocol_section}{algorithm_section}
...
def validate_generated_testbench_structure(tb_code: str, cand_idx: int) -> bool:
    """Reject common SystemVerilog or illegal Verilog-2001 constructs early."""
    stripped = tb_code.strip()
    checks = [
        (r'(?m)^\s*timescale\b',
         "Bare timescale directive without backtick"),
        (r'\bwhile\s*\(',
...
"""
    ...
    if not tb_code:
        return ""
    stripped = tb_code.strip()
    if not validate_generated_testbench_structure(stripped, cand_idx):
        return ""
    if re.search(r'(?m)^\s*timescale\b', stripped):
        print(f"      ⚠️  TB {cand_idx+1}: Bare timescale directive without backtick — discarding.")
        return ""
...
def autonomous_build_loop(
    ...
    for attempt in range(max_retries):
        ...
        if architecture_flawed:
            ...
            if not validate_architecture(architecture, expected_port_names=requested_port_names):
        ...
        if skip_reviewer:
            reason = ("skeleton/protocol design" if design_type != "generic"
