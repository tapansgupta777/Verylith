...
def classify_failure(log: str) -> str:
    ...
    l = log.lower()
    # Structural TB failures — testbench itself is broken, must regenerate
    ...
    # Any other "expected X got Y" mismatch is a logic failure — testbench is working
    if re.search(r'(expected|mismatch|fail)', l):
        return "LOGIC_MISMATCH"

    return "TB_STRUCTURAL"  # unrecognized — assume TB structural, regenerate
...
- SHIFT_LOGIC      → check the shift direction and concatenation: left shift with LSB input
                     is parallel_out <= {{parallel_out[6:0], serial_in}};
                     right shift with MSB input is parallel_out <= {{serial_in, parallel_out[7:1]}};
                     Verify direction matches the spec exactly.
...
- LOGIC_MISMATCH   → read the expected vs got values carefully. Identify which output
                     is wrong, trace back to its driver, and fix only that assignment.
- TB_TIMEOUT       → check for unintended combinational loops or missing $finish paths
- TB_STRUCTURAL    → likely a testbench issue; fix RTL defensively (clean interface)
...
def autonomous_build_loop(
    ...
    last_err_fingerprint  = None   # detect when the exact same error repeats
    same_err_count        = 0
    ...
    for attempt in range(max_retries):
        ...
        # TB LOCKING: The testbench is a fitness function. If the previous failure
        # was a logic mismatch (RTL produced wrong values), the testbench is working
        # correctly and must be locked. Only regenerate if the testbench itself was
        # structurally broken (syntax error, timeout, or completely unrecognized).
        _structural_failures = {"TB_TIMEOUT", "SYNTAX_ERROR", "TB_STRUCTURAL"}
        ...
        tb_is_logic_failure = (
            ...
            and tb_failure_type not in _structural_failures
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(reviewed_candidates)) as tb_executor:
            ...
            try:
                ...
                for f in tb_futures:
                    f.cancel()
                print(f"   ⚠️  Testbench executor timeout — {len(candidates)} testbench(es) collected before cutoff.")
        ...
        for v_code, tb_code in candidates:
            ...
            if not passed:
                ...
                # Track if the same error is repeating — if so, the locked testbench
                # may itself be the problem (broken golden model). Force TB regeneration.
                err_fp = code_hash(err)
                ...
                if same_err_count >= 4:
                    print(f"      🔁 Same error repeated {same_err_count}x — testbench golden model suspected. Unlocking TB.")
                    best_tb_code   = None   # force testbench regeneration next round
                    same_err_count = 0
