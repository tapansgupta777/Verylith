    ...
    for attempt in range(max_api_retries):
        try:
            if provider == "openai":
                ...
                try:
                    # PEER UPGRADE: Explicit timeout and native TimeoutError catching
                    timeout_val = kwargs.pop('timeout', 120)
                    response = client.chat.completions.create(**kwargs, timeout=timeout_val)
                except TimeoutError:
                    print(f"      ⚠️  Thread {cand_idx+1}: API timeout caught natively. Retrying...")
                    continue
...
def autonomous_build_loop(
    ...
    for attempt in range(max_retries):
        ...
        def _gen_tb(args):
            ...
            tb = generate_testbench(base_prompt, provider, design_name, v_code, best_err_log if best_tb_code else None, best_tb_code, idx)
            return idx, v_code, tb

        candidates = []  
        # PEER UPGRADE: Throttle TB generation to prevent API choking
        with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, min(2, len(reviewed_candidates)))) as tb_executor:
            tb_futures = [tb_executor.submit(_gen_tb, (idx, v_code)) for idx, v_code in enumerate(reviewed_candidates)]
            
            # PEER UPGRADE: Production-Safe loop wrapper to prevent complete crashes on TB timeouts
            try:
                for tb_future in concurrent.futures.as_completed(tb_futures, timeout=180):
                    try:
                        ...
                        print(f"      ⚠️  TB thread failed: {e}")
                        continue
            except concurrent.futures.TimeoutError:
                print("   ⚠️  Testbench generation timeout — continuing with completed TBs.")
