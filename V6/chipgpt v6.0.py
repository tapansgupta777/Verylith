...
def verify_with_yosys(v_file: str, base_prompt: str):
    ...
    if memoryish:
        yosys_script = (
            f"read_verilog {v_file}; proc; opt; memory_dff; memory_collect; "
            "check -assert; stat"
        )
    else:
        yosys_script = f"read_verilog {v_file}; proc; opt; fsm; memory; check -assert; prep; stat"
    ...
    try:
        ...
        if r.returncode == 0 and "ERROR" not in log:
            ...
            return True, f"Cells: {cells}, DFF cells: {dffs}{note_text}"
        return False, truncate_log(log, 20)
    except subprocess.TimeoutExpired:
        memoryish = any(
            kw in base_prompt.lower()
            for kw in ("memory", "sram", "ram", "bram", "byte_en", "byte enable", "mem_depth")
        )
        if memoryish:
            return False, (
